...
uv run --package osii-model-provider-bridge --extra dev \
  ...
  --app-dir osii-core/services/model-provider-bridge --port 8096
```
...
```powershell
uv run --package osii-model-provider-bridge --extra dev uvicorn tests.fake_openai_server:app --app-dir osii-core/services/model-provider-bridge --port 8096
...
uv run --package osii-model-provider-bridge python \
  osii-core/services/model-provider-bridge/scripts/live_openai_contract.py
```
