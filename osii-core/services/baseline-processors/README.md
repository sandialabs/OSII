...
This packaging directory consolidates six small baseline or HTTP-only
...
Available commands are `extractor`, `ocr`, `synthesizer`, `embedder`,
`enricher`, and `model-bridge`. The `ocr` command is ordinary page-by-page
Tesseract; it does not use the Toolbox processor's experimental OpenCV region
detection. For example:

```bash
podman run --rm -p 8092:8092 quay.io/your-org/osii-baseline-processors:0.1.0 extractor
podman run --rm -p 8080:8080 quay.io/your-org/osii-baseline-processors:0.1.0 ocr
podman run --rm -p 8085:8085 quay.io/your-org/osii-baseline-processors:0.1.0 embedder
```

The image contains the Tesseract executable and checked OCR language data, but
no LLM or embedding-model weights, Hugging Face client, Ollama runtime, or
