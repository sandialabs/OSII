...
from osii.domain.artifacts.object_source_state import get_object_source_state
...
def get_object_artifact_summary(osii_root: Path, file_id: str) -> dict | None:
    ...
    source_state = get_object_source_state(osii_root, file_id)

    has_extraction = len(manifest) > 0
    has_synthesis = len(syntheses) > 0
    has_enrichments = len(enrichments) > 0

    is_active = (source_state or {}).get("status", "active") == "active"
