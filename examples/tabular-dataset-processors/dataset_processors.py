"""Copyable Processor API examples for CSV data and dataset collections."""
    ...
    def extract(self, request: ExtractionRequest) -> ExtractionResponse:
        ...
        for source_line, raw_row in enumerate(reader, start=2):
            if len(rows) >= maximum_rows:
                truncated = True
                break
            row = {name: _value(raw_row.get(original) or "") for name, original in zip(names, reader.fieldnames, strict=True)}
            rows.append(row)
